...
int add_where_field(Command_t *cmd, const char *argument) {
    ...
    if (cmd->whe_args.fields) {
        memcpy(buf, cmd->whe_args.fields, sizeof(char*) * fields_len);
        free(cmd->whe_args.fields);
    }

    cmd->whe_args.fields = buf;
    cmd->whe_args.fields[fields_len] = strdup(argument);
...
void cleanup_Command(Command_t *cmd) {
    ...
    if (cmd->type == SELECT_CMD) {
        ...
        for (idx = 0; idx < cmd->whe_args.fields_len; idx++) {
            free(cmd->whe_args.fields[idx]);
            cmd->whe_args.fields[idx] = NULL;
        }
        free(cmd->whe_args.fields);
