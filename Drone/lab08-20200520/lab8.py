        ...
        for (faces_x, faces_y, faces_w, faces_h) in faces:
            for (rects_x, rects_y, rects_w, rects_h) in rects:
                if(faces_x > rects_x and faces_y > rects_y and (faces_x + faces_w) < (rects_x + rects_w) and (faces_y + faces_h) < (rects_y + rects_h)):
                    ...
                    ret, rvecs2, tvecs2 = cv2.solvePnP(objp, imgpoint_face, mtx, dist)
                    cv2.putText(img, str(tvecs2[2][0]), (faces_x + faces_w - 5, faces_y + faces_h - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2, cv2.LINE_AA)
                    #print(tvecs[2][0])
    cv2.imshow('img', img)
    cv2.waitKey(1)
