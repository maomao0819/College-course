...
<div>
  ...
  >
    <div>
      <label>
        <p class="label-txt"><b>Old password: </b></p>
        <input 
          id="old_password"
          name="old_password"
          type="password" 
    ...
    <div>
      <label>
        <p class="label-txt"><b>Confirm new password: </b></p>
        <input 
...
$("#form").submit(function(e) 
{
  ...
  else 
  {
    ...
    ({
      ...
      {
        if (data.includes('password is incorrect')) 
        {
          Swal.fire
          ({
            icon: 'warning',
        ...
        if (data.includes('Change the password successfully')) 
        {
          ...
          {
            if (result.value) 
              window.location = '/HW3/views/blog.php'
          })
        }
      }
    });
